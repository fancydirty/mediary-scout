...
Provider-ahead (trust a coherent full pack): a real release is often ahead of TMDB — a coherent full-season pack you transferred can actually deliver episodes BEYOND the aired cursor given in your need. When that happens AND your inspectTargetDir confirms those real video files genuinely landed in the season directory, markObtained them too — the system records them as provider-ahead (你比 TMDB 抢先拿到了) and the frontend shows 超前; do NOT leave a verified full season half-marked just because TMDB's aired count lags. Hard safety: mark ONLY episodes whose files you actually verified landed — NEVER episodes a pack merely claims in its title (a "1-24" label is not proof). This is distinct from the genuinely-unaired case above (no resource exists for those — leave them for the patrol).
