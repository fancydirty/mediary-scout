 ...
 *
 * 数据源:从主站 `site/data/posters-fallback.json` **拷来的快照**。
 *
 * **为什么不是共享同一份**:worker bundle 读不到 site/ 的静态文件,而为一份
 * 纯装饰性的兜底底图引入构建期 JSON 导入或生成脚本,复杂度不划算。
 *
 * **因此接受漂移,并明确同步责任**:两边各自演进不会有任何功能后果 ——
 * 这些海报只在 hero 背景里被遮罩盖掉大半,是氛围而非信息。
 * 若哪天要让两边一致,手动更新这个数组即可(28 条,一次 copy-paste)。
 * 不要在这上面加自动化,那是为装饰品付工程债。
 *
