...
- **Payment info** — handled by Paddle (Merchant of Record). We **never touch** your payment credentials — whatever method you use, the card or wallet details go to Paddle, not to us. We only receive a transaction ID and the payment email from Paddle.
...
- **付款信息**:由 Paddle(Merchant of Record)处理。我们**不接触**你的任何付款凭据 —— 无论你用哪种支付方式,卡号或钱包信息都只到 Paddle,不到我们这里。我们只从 Paddle 收到交易号与付款邮箱。
