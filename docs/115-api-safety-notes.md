...
per round at depth 1 for every file (stop: all landed / 8 idle rounds / 8 + N
rounds / the transfer line), and cancels the unlanded tasks with a single
`task_del`. Cost ≤ 2N + 12 for N files, ~N + 30 in practice; the per-file
