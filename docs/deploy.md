...
光鸭云盘(迅雷旗下,2026 年上线)和 115、123 一样走两条路:

- **光鸭分享链**(`guangyapan.com/s/…`):PanSou 搜到的光鸭分享,服务端直接转存进你的盘,文件夹整个转,几秒就完成。
- **磁力 / ed2k / BT**:PanSou(magnet 类型)和可选 Prowlarr 找到的磁力,经光鸭的离线下载拉进盘里。

> ⚠️ PanSou 上的光鸭分享有不少打得开却列不出文件(可能是分享者设了限制,或内容在审核),这种会明确报 `GUANGYA_SHARE_EMPTY`,agent 会换下一个候选。别的网盘(115 / 夸克 / 123 / 天翼)的分享链落不到光鸭,会报 `GUANGYA_UNSUPPORTED_LINK`。想要磁力覆盖更全,可以再配 **Prowlarr**。
>
